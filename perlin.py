...
def create_perlin_noise(seed=None, color=True, batch_size=11, image_size=28, normalize=True, precalc_fade=None):
    """
    ...
    if color:
        ...
        output = np.empty((batch_size, pixels, pixels, 1), dtype=np.float32)
    if color:
    ...
    # Normalize to L2=1
    if normalize:
        normalize_term = np.linalg.norm(output.reshape(batch_size, -1), axis=1)
        normalize_term = normalize_term[:, np.newaxis, np.newaxis, np.newaxis]
        output /= normalize_term
    return output
